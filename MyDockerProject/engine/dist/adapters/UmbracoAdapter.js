...
class UmbracoAdapter {
    ...
    async getProduct(productId) {
        try {
            ...
            if (hotelsResponse2.ok) {
                const hotels = await hotelsResponse2.json();
                for (const hotel of hotels) {
     ...
     */
    async getRoomAvailability(request, hotelId) {
        ...
        try {
            ...
            const response = await fetch(`${UMBRACO_API_BASE}/hotels/${hotelId}/availability?from=${fromStr}&to=${toStr}`);
            if (!response.ok) {
     ...
     */
    async getEventAvailability(request, product) {
     ...
     */
    async getPriceForDate(productId, date, productType) {
     ...
     */
    generateBasicAvailability(request) {
        const days = [];
        const fromDate = new Date(request.from);
        const toDate = new Date(request.to);
        let currentDate = new Date(fromDate);
...
exports.UmbracoAdapter = UmbracoAdapter;
